...
async function createJsonSchemaCompletion(
  ...
  return ai.chat.completions.create({
    ...
    response_format: {
      type: 'json_schema',
      json_schema: EXTRACTED_DATA_JSON_SCHEMA,
    },
  } as any);
}

async function createJsonObjectCompletion(
  ai: OpenAI,
  messages: Array<{ role: 'user'; content: any }>
) {
  return ai.chat.completions.create({
...
async function createStructuredCompletion(
  ...
  try {
    return await createJsonSchemaCompletion(ai, messages);
  } catch (error) {
    if (!isStructuredOutputUnsupported(error)) throw error;

    console.warn(
      `[${OCR_MODEL}] json_schema response_format unsupported. Falling back to json_object response_format.`
    );
    try {
      return await createJsonObjectCompletion(ai, messages);
    } catch (fallbackError) {
      if (!isStructuredOutputUnsupported(fallbackError)) throw fallbackError;

      console.warn(
        `[${OCR_MODEL}] json_object response_format unsupported. Falling back to prompt-only JSON repair.`
      );
      return createOcrCompletion(ai, messages);
    }
  }
}

async function createOcrCompletion(
  ai: OpenAI,
  messages: Array<{ role: 'user'; content: any }>
) {
  return ai.chat.completions.create({
...
async function callMimoVision(ai: OpenAI, file: Express.Multer.File) {
  ...
  const visionContent = await createVisionContent(file);

  for (let i = 0; i < retries; i += 1) {
    try {
      const response = await createOcrCompletion(ai, [
          {
            role: 'user',
